...
class Tokenizer {
  ...
  pushToken(token2, pushToBracketStack = false) {
    if (pushToBracketStack) {
      this.bracketStack.push([token2.type, this.index]);
    }
...
`) {
            ...
            const canAppend = token ? token.text.startsWith(`
`) : false;
            const lastToken = canAppend ? token : this.tokens.last(2);
...
`;
              token2.type = type;
              this.tokens[this.tokens.length - 1] = token2;
            } else {
              const token2 = new Token(type, `
`, start);
              this.tokens.push(token2);
            }
          } else {
...
class Parser {
  ...
  shouldParseInfixOrPostfix(token2, scope) {
    if (!token2)
      return false;
    if (token2.type != "Identifier")
      return false;
    const scopeElement = scope.readElement(token2.text);
...
// src/app.ts
function renderToken(token2) {
  const type = token2.text != "Array" && token2.text != "Map" ? token2.type : "Hint";
  return `<span data-index="${token2.index}" class="_Token _${type}">${token2.text.replaceAll(`
